...
export const StatusHistoryCard = ({ statusHistory }: StatusHistoryCardProps) => {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle>{t('invoices.detail.statusHistory.title')}</CardTitle>
          </div>
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <Badge variant="outline">{getStatusHistoryLabel(entry.status)}</Badge>
                    ...
                    <span className="text-sm text-muted-foreground">
                      {entry.updated?.length
                        ? t('invoices.detail.statusHistory.changes', { count: entry.updated.length })
                  ...
                  {entry.updated?.length ? (
                    <div className="mt-4 space-y-2">
                      {entry.updated.map((change, changeIndex) => (
                        ...
                        >
                          <div className="font-medium text-foreground">{change.item}</div>
